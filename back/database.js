...
async function processStdinCommand(cmd){
	...
	if(cmd === 'erase_db_and_start_clean'){
		await rw.deleteTable("images",rethinkdb); // ignore errors if the table does not exist, this is a setup function
		await rw.createTable("images",rethinkdb);
		await rw.deleteTable("users",rethinkdb); // clear user table.
		await rw.createTable("users",rethinkdb);
		await rw.deleteTable("recipes",rethinkdb);
		await rw.createTable("recipes",rethinkdb);
